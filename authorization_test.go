...
func TestParseCredential(t *testing.T) {
	...
	credential, err := ParseCredential([]byte(expect))
	assert.NoError(t, err)

	assert.Equal(t, "AKIAIOSFODNN7EXAMPLE", string(credential.AccessKeyID))
	assert.True(t, time.Date(2013, 5, 24, 0, 0, 0, 0, time.UTC).Equal(credential.Date))
	assert.Equal(t, "us-east-1", string(credential.Region))
	assert.Equal(t, "s3", string(credential.Service))
	assert.Equal(t, "aws4_request", string(credential.Method))
...
func TestParseAuthorization(t *testing.T) {
	...
	auth, err := ParseAuthorization([]byte(expect))
	assert.NoError(t, err)

	assert.Equal(t, "AWS4-HMAC-SHA256", string(auth.Method))
