...
func bucketFromPath(r *http.Request) (string, error) {
	...
	if bucketName == "" {
		return "", &Error{
			...
			Message:   "Missing bucket name in request URL path.",
		}
	}
...
func bucketFromRequest(r *http.Request, domain []string) (string, error) {
	if len(domain) == 0 {
